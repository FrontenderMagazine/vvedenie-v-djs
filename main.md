...
представление данных - самых эффективный способ сделать анализ большого обыема
данных. Пытаясь найти оптимальный способ представить информацию на практике, 
я наткнулся на библиотеку d3.js и понял, что именно на ее основе можно в полной
мере воплотить мои идеи.

d3 расшифровывается как data driven document. Это JavaScript библиотека
ориентированая на работу с данными и их визуальное представление для веба,
...
[Множество][2] библиотек основаных на d3.js предоставляет готовый функционал
...
веб-станбартами, что дает возможность сосредоточиться на самих данных и
...
.enter() - вернет зарезирвированные новые места для элементов с новыми, уже
    привязанными данными (данные пишуться в __data__) и вернет ссылку на них
    (это наша новая выборка зарезирвированных элементов с привязанными к ним
    данными).
    [{__data__: 16}, {__data__: 23}, {__data__: 42}]

.append() - для каждого несушествующего элемента создается элемент и
...
Всё это, при детальном рассмотрении дает широкие возможности в плане
...
![Схема][Изменение содержимого по обновленным данным]
...
В выборку enter() ушел один элемент. Добавим его.
...
## Еще раз про три состояния enter, update, exit
...
елементы попадают в exit() выборку и удаляются.
...
Пробуйте все изложенное своими руками. 
...
[Изменение содержимого по обновленным данным]: img/04.svg "Изменение содержимого по обновленным данным"
